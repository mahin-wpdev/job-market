...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_test'])) {
    ...
    if ($id) {
        $conn->query("UPDATE tests SET title = '$title', passing_score = $pass WHERE id = $id");
    } else {
        $conn->query("INSERT INTO tests (title, passing_score) VALUES ('$title', $pass)");
    }
    header("Location: tests.php");
}
            ...
            <tbody>
                <?php while ($t = $tests->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#testModal" onclick="editTest(<?= htmlspecialchars(json_encode($t)) ?>)">Edit</button>
            ...
            <div class="modal-content">
                <form method="POST">
                    <div class="modal-header">
                        <h5>টেস্ট তথ্য</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <input type="number" name="passing_score" id="passing_score" class="form-control" placeholder="পাসিং স্কোর (1-100)" value="80">
                    </div>
                    <div class="modal-footer"><button type="submit" name="save_test" class="btn btn-primary">সংরক্ষণ</button></div>
                </form>
            </div>
        </div>
    </div>
